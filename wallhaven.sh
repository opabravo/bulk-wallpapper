...
function downloadWallpapers {
    ...
    for imgURL in $URLSFORIMAGES
        do
            if grep -w "$imgURL" xdownloaded.txt >/dev/null
            then
    ...
    then
        ...
        SHELL=$(type -p bash) parallel --gnu --no-notice \
            'imgURL={} && ! downloadWallpaper $imgURL && echo $imgURL >> xdownloaded.txt' < download.txt
            rm tmp download.txt
        else
            rm tmp
    fi
...
if [ ! -f ./xdownloaded.txt ]
then
    touch xdownloaded.txt
fi
